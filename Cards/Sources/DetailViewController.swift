...
internal class DetailViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if let detail = detailView {
            let detailHeight = (isFullscreen) ? detail.frame.height: (detail.frame.height * 0.6)
            ...
            scrollView.contentSize = CGSize(width: scrollView.bounds.width, height: detail.frame.midY)
    ...
    func layout(_ rect: CGRect, isPresenting: Bool, isAnimating: Bool = true, transform: CGAffineTransform = CGAffineTransform.identity){
        ...
        if isFullscreen {
            ...
            scrollView.frame.size = CGSize(width: LayoutHelper.XScreen(90), height: LayoutHelper.YScreen(70) - 40)
